...
try {
    $response = Invoke-WebRequest -Uri "http://localhost:8081/api/auth/register" -Method Post -Body '{"fullName":"Jane Doe","email":"jane@doe.com","password":"password123","role":"USER"}' -ContentType "application/json"
...
try {
    $response2 = Invoke-WebRequest -Uri "http://localhost:8081/api/auth/login" -Method Post -Body '{"email":"jane@doe.com","password":"password123"}' -ContentType "application/json"
